...
function createOpenSearchClient() {
    // For serverless, we need AWS signature authentication
    return new Client({
        ...AwsSigv4Signer({
            region: 'us-east-1', // Serverless collection is in us-east-1
...
exports.handler = async (event) => {
    ...
    try {
        ...
        const indexName = 'georgia-statutes'; // Using the index name you created
        ...
        // Create index with correct mapping for serverless
                ...
                mappings: {
                    properties: {
                        embedding: {
                            ...
                            method: {
                                name: 'hnsw',
                                space_type: 'innerproduct', // For dot product similarity
                                engine: 'nmslib'
                            }
                        },
                        title: { type: 'text' },
        ...
        for (const statute of georgiaCode) {
            try {
                ...
                successCount++;
                console.log(`Successfully indexed: ${statute.citation}`);
