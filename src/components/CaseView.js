...
export default function CaseView() {
  ...
  const handleDeleteDocument = async (doc) => {
    ...
    try {
      // Use the document key for deletion
      const res = await fetch(`${API_BASE}/cases/${id}/documents/${encodeURIComponent(doc.key)}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center">
                  <div className="text-2xl font-bold text-blue-600">
                    {documents.length + (cs.documents || []).length}
                  </div>
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-purple-600">
                    {cs.createdAt ? new Date(cs.createdAt).toLocaleDateString() : 'N/A'}
                  </div>
            ...
            <div className="card-body">
              <div className="space-y-2">
                ...
                <button 
                  ...
                  onClick={() => alert('Case notes feature coming soon!')}
                >
                  📝 Add Notes
                </button>
                <button 
                  className="btn btn-warning btn-sm w-full"
                  onClick={() => alert('Share case feature coming soon!')}
                >
                  👥 Share Case
                </button>
              </div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <div className="text-sm">
                    ...
                    <div className="text-gray-500">
                      {cs.createdAt ? new Date(cs.createdAt).toLocaleDateString() : 'Recently'}
                    </div>
                  </div>
                </div>
                {documents.length > 0 && (
                  <div className="flex items-center space-x-2">
                    ...
                    <div className="text-sm">
                      ...
                      <div className="text-gray-500">Recently</div>
                    </div>
                  </div>
                )}
                {folders.length > 0 && (
                  <div className="flex items-center space-x-2">
                    ...
                    <div className="text-sm">
                      <span className="font-medium">{folders.length} folders created</span>
                      <div className="text-gray-500">Recently</div>
                    </div>
                  </div>
                )}
          ...
          <div className="flex-1">
            ...
            <div className="card-body">
              ...
              {documents
                .filter(doc => {
                  if (!currentFolder) {
                    return true; // Show all documents when no folder selected
                  }
              ...
              {/* Test documents to verify delete buttons work */}
              {[
                  { id: 'test1', filename: 'Test Document 1.pdf', uploadedAt: '2025-01-01', key: 'test1.pdf' },
                  { id: 'test2', filename: 'Test Document 2.pdf', uploadedAt: '2025-01-02', key: 'test2.pdf' }
                ].map(d => (
                ...
                <button 
                  onClick={() => {
                    if (window.confirm(`Are you sure you want to delete ${showingVersionsFor.filename}?`)) {
                      handleDeleteDocument(showingVersionsFor);
